...
* メモ内の `[[key]]` 形式のリンクから、別のノートを**簡易プレビュー**（読み取り専用ウィンドウ）で表示。
* ノート詳細表示時に、そのノートを引用している他のノート（被リンク元）を自動でリストアップ。
* 現在の検索結果に基づき、ノート間のリンクを視覚化する「**グラフ表示**」機能（ノードの**ダブルクリックでPDFを開く**ことが可能）。
* 現在の検索クエリを保存・管理・呼び出しできる「**保存済み検索（スマートフォルダ）**」機能。
* 現在の検索結果や全ノートからランダムでノートを表示する「**ランダムノート（閃き）**」ボタン。
* 現在の検索結果を「**エクスポート**」する機能（メタデータCSV、本文TXTファイル群、グラフHTML、検索条件TXTをフォルダにまとめて保存）。
* ノートのメタデータ（メモ、タグ、Index Key）を**DBに直接書き込み・編集・削除**する手動操作機能。
...
3.  リストに表示されたPDFをクリック（または複数選択して「一括編集」）し、タグ、メモ、Index Keyを編集します。
4.  (オプション) `リスト保存 (CSV)` / `リスト読込 (CSV)` をクリックすると、現在の編集状態（メタデータのみ）を中間ファイルとして保存/読み込みできます。
5.  「統合PDFを生成」をクリックし、保存場所と年月を指定すると、統合PDFが生成されます。
    * この瞬間に、PDFから**本文テキスト（`full_text`）が抽出**され、目次情報と共に **マスターDB (`config.ini` で指定したパス) に追記**されます。
...
7.  「検索保存」でよく使う検索を登録したり、「エクスポート」で結果を外部に出力したり、「閃き」ボタンで過去のノートと思いがけず再会できます。
...
`Synapsen Nexus` が「グラフ表示」機能 で生成する `synapsen_graph.html` ファイルは、`Synapsen` プログラムの「出力」であり、AGPL-3.0 ライセンス の対象外です。

このHTMLファイルは、`pyvis` ライブラリ（MITライセンス） と、ユーザー自身のノートデータ（タイトルやリンク構造）で構成されています。したがって、ユーザーは生成された `synapsen_graph.html` をAGPL-3.0 を気にすることなく自由に公開・配布できます。
...
そしてWebクリップ機能とMarkdown変換を実現する **Playwright** および **Pandoc** の開発者コミュニティに心から感謝申し上げます。
