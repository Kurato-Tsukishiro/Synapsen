...
* **ノートの配置:** Nexusの「ノート追加」機能や、検索結果からノートをカードとしてキャンバスに追加します。
* **視覚的なリンク（コネクタ）:**
...
* **付箋 (Sticky Note) 機能:**
    ...
    * **まとめノート作成:** 複数の付箋を選択して「まとめてPDFを作成」することで、グルーピングしたアイデアを1つのノートに集約できます。
* **図形描画:** 枠線（ラベリング用）や直線を描画して、情報を構造化できます。カード以外のこれらのアイテムも自由に選択・移動可能です。
* **Nexus連携:** キャンバス上で選択した複数のノートに対して、Nexus側で「OR検索」を即座に実行できます。
* **PDF出力:** キャンバスの状態をそのままPDFとして保存できます（Normalisiererでの再処理時に縮小されないよう、特殊なメタデータが付与されます）。
