...
* **Server Color: 錆浅葱 (Sabi-asagi)**
    * **Hex:** `#5C9291`
    * **意味:** 静寂、忍耐、不屈の心、持続する愛
    * **役割:** **Web Server** / 静寂の中で揺るぎなく稼働し、忍耐強く積み上げられた知恵を、途切れることのない繋がりとして共有し続けます。

離れた場所からでも静かに知へと繋がる、ネットワークの架け橋を象徴します。
...
* **D&D / ペーストによるクリップ:**
    ...
    * **PDFのページ指定による部分抽出**: PDFを処理対象リストに追加後、専用の入力欄から特定のページだけを抜き出して正規化（または結合）することができます。
        > **💡 Tips: PDFの特定ページだけをノート化したい場合**  
        > リスト上のページ指定欄に以下のように入力することで、必要なページだけを抽出して処理できます。すでに正規化済みのPDFの場合、サイズの再変換はスキップされ、高速に切り出し・結合のみが行われます。
        > - **指定例:**
        >   - `3` （3ページ目のみ）
        >   - `1-15` （1ページ目から15ページ目まで）
        >   - `1, 20-35` （1ページ目と、20〜35ページ目）
    * **タグ情報の自動埋め込み:**
...
Synapsen Normalisierer でファイル（PDFや画像、Markdownなど）を正規化する際、ファイル名に **`[タグ名]`** を含めることで、PDFのメタデータに自動でタグを埋め込むことができます。この機能により、ファイル作成時にタグを設定する事が可能になり、スムーズな分類が可能です。

* **指定フォーマット:**
    * ファイル名の任意の場所に、大括弧 `[]` で囲んだタグを記述します。カンマ `,` 区切りで複数のタグを同時に指定することも可能です。
    * **例:** `20241025_120000_Synapsen設計書[ZTypes_Permanent, Concepts_Idea].pdf`
* **エイリアス（別名）の活用による表記ゆれ防止:**
    * `PDFTags.txt` にエイリアス（別名）を定義しておくことで、ファイル名には直感的な日本語（略称など）を使いつつ、システム内部では正式なタグ名として統一して登録することができます。
* **`PDFTags.txt` の設定例:**
    ```text
    # 正式タグ名 [エイリアス1, エイリアス2...] の形式で記述します
    ZTypes_Literature [読書ノート, 読書メモ]
    ```
    - 上記のように設定した場合、入力ファイル名を `アジャイル開発の実践[読書メモ].pdf` として正規化すると、Ersteller で読み込んだ際には自動的に正式な **`ZTypes_Literature`** タグに変換されて登録されます。（大文字・小文字の表記ゆれも自動で吸収されます）
    - **💡 Tips: タイトルでのタグの可視化**
      > - ファイル名に指定した `[タグ名]` の文字列は、PDFのタイトル（目次）としてそのまま維持されます。
      > - これにより、統合PDFの目次を確認した際に「どのタグが付与されたノートか」が一目でわかるという副次的なメリットもあります。

#### Synapsen独自のMarkdown拡張機能

##### 📝 台本・対話形式のPDF出力

Synapsenは、Markdownの**定義リスト記法**を応用して、インタビューや台本のような「対話形式」のレイアウトを美しくPDF化する機能を備えています。

**書き方:**
  - 話者名の次の行に、半角コロン `:` と半角スペース ` ` を置いてセリフを書きます。
  ...
  - [サンプルファイル (Markdown)](<samples/20250721_025900_投与設計の第一歩 -分布容積の理解-_資料.md> "20250721_025900_投与設計の第一歩 -分布容積の理解-_資料.md")

##### 📄 改ページ制御

Synapsenは、MarkdownファイルやCanvasの付箋機能からPDFを生成する際、自動および手動での柔軟な改ページ（ページ分割）レイアウトに対応しています。

* **見出しによる自動改ページ**
  * 見出しレベル1 (`# 見出し名`) を記述すると、その直前で自動的に改ページが行われます。
  * （※ ドキュメントの最初の見出しや、付箋ノート作成時の `# [内容]` ヘッダーなど、文脈上分割すべきでない箇所は自動的に保護され、連鎖的な改ページは防がれます）
* **手動（強制）改ページタグ**
  * Markdown内の任意の場所に `<!-- BP -->` (Break Page) と記述することで、その場所で強制的に改ページを行うことができます。長文のメモを区切る際や、特定のトピックを新しいページから始めたい場合に便利です。
  > * サンプルとして、ReadMeでこのタグを使用しています。

#### 🖇 外部OCR連携（透明テキスト自動統合）

Synapsenは、TesseractやOllamaを用いた内蔵のOCR機能に加え、外部のOCRツール（Yomitokuなど）で生成された高精度なテキストデータを自動的に読み込み、正規化後のPDFに統合する機能を備えています。

この機能により、Synapsen本体とは独立した外部プロセスとしてお好みのツールを実行し、標準的なPDFフォーマットを介してシームレスかつクリーンに連携・活用することが可能です。

#### 使い方
処理対象のファイル（例: `YYYYMMDD_hhmmss_Name.pdf` または画像ファイル）と同じディレクトリに、外部ツールで出力した `_ocr.pdf` を配置してSynapsenに読み込ませます。

1. **ファイル名の準備**
   - 対象ファイル: `YYYYMMDD_hhmmss_Name.pdf`
   - OCR済ファイル: `YYYYMMDD_hhmmss_Name_ocr.pdf`
2. **自動処理**
   上記の状態でSynapsenの正規化パイプライン（D&Dまたはフォルダ解析）を実行すると、以下の処理が自動で行われます。
   - `_ocr.pdf` 側から透明化テキスト（OCR結果）とその座標情報を抽出します。
   - 抽出したテキストを、出力される `YYYYMMDD_hhmmss_Name.pdf` に適切なスケール（ページサイズの比率）で動的調整して埋め込みます。
   - `_ocr.pdf` 自身は正規化処理から自動的に除外されるため、ファイルが二重に生成されることはありません。
   - 外部OCRのテキストが統合された場合、内蔵OCR（Tesseract等）の処理は自動でスキップされます。

#### メリット
* **高精度なレイアウト保持:** 外部ツールが解析したテキストボックスの座標をSynapsenの座標系へ動的にスケーリングするため、テキストの位置ズレが発生しません。
* **独立したプロセス連携:** 外部ツールをSynapsenの内部に組み込まない「ファイルベースでの疎結合な連携」であるため、本体のシステム環境に影響を与えることなく、不要な依存関係を持ち込まずに独立して安全な運用が可能です。
...
* **メタデータの自動抽出:**
    ...
    * **5. Canvas接続情報の自動継承:**
        * 読み込んだPDFが Canvas 全体を出力したもの（`STypes_Canvas` タグを持つもの）である場合、内部のキャンバスデータを自動解析します。
        * キャンバス上で接続されていたノート同士（および `[[YYYYMMDDhhmmss]]` のような予約キーをタイトルに持つ付箋）のリンク関係を抽出し、該当する各ノートのメモ欄に相互リンクを自動で追記します。これにより、Canvas上での視覚的な思考整理が、そのままデータベースのリンク構造として反映されます。
...
* **高度な検索:**
    ...
    * **高度な日付検索**
      * **範囲検索**
        * `date:>=YYYYMMDD` (以降), `date:<=YYYYMMDD` (以前), `date:YYYYMMDD-YYYYMMDD` (期間) などの日付範囲検索が可能。
        * `date:>=2024` (年指定/以降), `date:<=202303` (年月指定/以前), `date:>2024` (より後), `date:<2023` (より前) など、省略記法にも対応。
      * **年を指定しない検索**
        * `date:12-31` : 毎年12月31日のノートを検索 (記念日や振り返りに)
        * `date:12-`   : 毎年12月のノートを全て検索 (季節性のタスクや傾向分析に)
    * PDF本文（`full_text`）を検索対象に含める「**本文検索**」チェックボックス。
    * 「保存済み検索」でよく使う検索を登録可能。
* **タグリスト (Tag Explorer):**
    * 検索バー横の「Tag」ボタン（または `Ctrl+T`）で、登録されている全タグを別ウィンドウで一覧表示します。
    * **ソート機能:** 「件数順（頻出タグ）」や「名前順（辞書順）」で並び替えができ、データベース全体の傾向を俯瞰できます。
    * **クエリ作成支援:**
        * タグをクリックすると、検索バーに自動入力されます。
        * ウィンドウ内の「AND / OR」スイッチを切り替えることで、現在の検索ワードに対して「絞り込み (AND)」するか「追加 (OR)」するかを直感的に制御できます。
        * キーボードショートカット (`Tab` でAND/OR切替、`Alt+S` でソート順切替など) にも対応しており、高速な操作が可能です。
* **キーボード操作の強化:**
...
* **メタデータの編集と管理:**
    ...
    * **一括編集 (Batch Edit):**
        ...
        * **メモ編集:** テキストやリンクを一括で追記できます。誤消去を防ぐため、`Ersteller`の一括編集とは異なり「既存メモへの追記（Append）」モード固定で動作します。
    * **⚠️ データの保存について:**
        * Nexusで行った編集は、検索用の**マスターデータベースのみ**に保存され、PDFファイル自体には即時反映されません。
        * 編集内容をPDFファイル（長期保存用）にも反映させるため、定期的に `Ersteller` の **[メタデータ同期 (DB→PDF)]** を実行することを推奨します。
        * 運用の詳細は [実践ガイド: 手書きノートと知識の循環](#実践ガイド-手書きノートと知識の循環) を参照してください。
...
* **付箋ノートの直接作成 (Sticky Note):**
    * 詳細パネルの「付箋ノート作成」ボタンから、ローカルのファイル（`.md` や `.txt`）を選択し、背景色と Index Key を指定することで、即座に「付箋ノート」としてPDF化・正規化できます。
    * **タグ設定:** 作成時にタグを設定する事ができます。正規化のタイミングで埋め込まれ、統合時に読み込み設定します。
    * **引用関係の自動構築:** 作成された付箋のメタデータには、現在詳細パネルで表示しているノートが「引用先 (References)」として自動的に設定・記録されます。これにより、後から `Ersteller` で取り込んだ際にノート間のリンク関係が自動で構築されます。
    * **出力先ディレクトリのフォールバック仕様:** 作成されたPDFの保存先は、`config.ini` の設定に基づいて以下の優先順位で自動判別されます。これにより、設定漏れによる出力エラーを防ぎます。
        1. **優先:** `[Watchdog]` セクションの `output_dir` (自動監視ツールとのシームレスな連携)
        2. **代替:** 上記が存在しない場合は、`[Paths]` セクションの `nexus_output_folder`
...
* **補遺:**
    ...
    * 検索バー横の「Tag」ボタンからタグリストを呼び出し、クリック入力でタグ検索を行うことができます。
...
* **付箋 (Sticky Note) 機能:**
    ...
    * **ノートへの昇格:** 付箋を右クリックして「PDFを作成」すると、正規化済みのPDFノートとして出力されます。
        ...
        * **予約キーを含むタイトルに変換:** 作成後Erstellerの処理を通り、データベースに統合された後に再度Canvasデータを読み込むと、付箋は「ノート」に変換されています。
    ...
    * **予約キーによるノート自動変換:** 
        * 付箋のタイトルを `[[YYYYMMDDhhmmss]]` または `[[YYYYMMDDhhmmss:任意のタイトル]]` の形式で入力すると、プレースホルダーとして機能します。
        * 後日、Erstellerで該当するキーのノートがデータベースに統合された後にCanvasデータを再度読み込むと、対象の「付箋」が自動的に「ノート」へと変換され、接続情報もそのまま引き継がれます。
        * **💡 設計思想:** 
          * 本ツールのコンセプト元である『情報は1冊のノートにまとめなさい』で紹介されている「紙資料をノートに貼り付けて使用する方法」のCanvas上での応用です。
          * **正規化はしたが、まだErstellerによる統合処理を受けていない画像クリップやPDFの切り抜き等、Canvasでの整理中に追加した実体のない資料**を付箋としてCanvas上に「疑似的に貼り付け（仮置き）」しながら、思考や構造化を止めることなく進めることができます。
...
* **PDF出力:** キャンバスの状態をそのままPDFとして保存できます。
    ...
    * **タグの自動付与:**
      * `SType_Canvas` タグが付与されるようメタデータが埋め込まれます。
      * Normalisiererでの再処理時に縮小されないよう、特殊なメタデータも付与されます。
    * **接続線のデータベース同期:** キャンバス上でノートや予約キー付箋同士を結んだ「接続線（コネクタ）」の情報もメタデータに完全に保存されます。このPDFを `Ersteller` で取り込むことで、繋いだノート同士のメモ欄にリンクが自動生成・追記されるため、Canvasを「未統合ノートの事前リンク作業ツール」としても強力に活用できます。
* **キャンバスデータの読み込み・復元:** 保存したJSONファイルや、エクスポートされたPDFファイル（または統合PDF）からキャンバスの状態を直接読み込んで復元できます。
    * **複数データからの選択:** 統合PDFのように複数のキャンバスデータがファイル（バックアップ）に含まれている場合、タイトルやキーを基に復元対象を選択できる専用のダイアログが表示され、希望のキャンバス状態を復元できます。
...
| `Ctrl + A` | すべて選択(ページ) | 現在表示しているページのノートを、すべて選択状態にします。 |
| `Ctrl + Shift + A` | すべて選択(検索結果) | 検索結果のノートを、すべて選択状態にします。 |
| `Ctrl + D` | 選択解除 | 選択状態をすべて解除します。 |
| **🛠️ 編集・アクション** |
| `Ctrl + E` | 編集 | リストで選択しているノートの編集画面を開きます。 |
| `Ctrl + G` | 付箋ノート生成 | リストで選択しているノートに対して付箋ノートを作成する為の画面を開きます。 |
| `Ctrl + Shift + E` | 一括編集 | リストで選択状態のノートに対して、一括編集画面を開きます。 |
| `Ctrl + Enter` | Canvasへ送る | リストで選択状態のノートをCanvasに追加します。(Canvas未起動時はバックグラウンドで起動) |
...
| キー | 機能 | 説明 |
| :--- | :--- | :--- |
| ``←`` (左矢印) | ページ移動 | 前のページへ移動します。 |
| ``→`` (右矢印) | ページ移動 | 次のページへ移動します。 |
| ``Enter`` | PDFを開く | 詳細表示中のPDFを、外部ビューアで開きます。 |
| ``Space`` | 表示モード切替 | 縮小/拡大表示 (モード) を切り替えます。 |
| `L` | 関連グラフ | 詳細表示中のノートを中心とした関連グラフを表示します。 |
| `Ctrl + E` | 編集 | 選択中のノート編集画面を開きます。 |
| ``Esc`` | 閉じる | プレビューウィンドウを閉じます。 |

### Nexus (タグリスト画面)
| キー | 機能 | 説明 |
| :--- | :--- | :--- |
| `Tab` | 検索条件切替 | 検索条件の設定 ("AND"/"OR") を切り替えます。 |
| `Alt + S` | ソートオーダー切替 | タグの並び順（昇順/降順）を切り替えます。 |
| `Alt + M` | ソートキー切替 | タグの並べ方（件数/名前）を切り替えます。 |
...
2.  **清書と要約 (Permanent Note):**
    ...
    * このノートは **Markdown (.md), Word (.docx), テキスト (.txt)** など、お好みの形式で作成してください。
        ...
        * **Canvas機能の活用**
            * NexusのCanvas機能を用いて、情報のつながりを視覚的に整理・表現することも有用です。
            * Canvasの「付箋」機能を使って要約や清書を行うこともできます。付箋はMarkdown記法に対応しており、そのまま正規化済みのPDFとして出力可能です。
            * **知識の循環を支える再編集機能:** 
                > 出力されたPDFにはCanvasの構成データ（JSON）が埋め込まれており、このデータは `Ersteller` による統合後の「統合PDF」にもそのまま引き継がれます。<br>
                > そのため、元のJSONデータが手元になくとも、後日大本の統合PDFから直接Canvasを開いて再編集することが可能です。<br>
                > 単なる出力直後の手直しではなく、時間が経ってからの情報の更新やまとめの再作成といった、長期的な「知識の循環」を意識した運用が行えます。
...
6.  考えがまとまったら、成果を正規のノートファイル (PDF) として出力・保存します。
    * **個別のアイデア:** 付箋（または選択した複数の付箋）を右クリックして「PDFを作成」します。自動で `SType_Sticky` タグが付与されます。Markdown記法（見出し、リストなど）も反映されます。
    * **全体構造:** ツールバーの「PDF出力」から、配置したカードや接続線を含めたホワイトボード全体を1つのPDFとして保存します。自動で `SType_Canvas` タグが付与されます。
    * 出力時、Index Keyを選択することで自動的にQRコード等のメタデータが埋め込まれます。
7.  保存されたPDF (`Nexus_Output` フォルダ内) は `Ersteller` で取り込むことで、次回の統合PDFに含まれ、知識ベースの一部となります。

> [実践ガイド: 手書きノートと知識の循環](#実践ガイド-手書きノートと知識の循環) に、再編集機能を含めた長期的な活用法を記載しています。
...
> もし、ユーザーが**変更を加えた生成スクリプト自体を再配布・公開する場合**は、`AGPL-3.0` の条項に従う必要があります。
>
> **生成されるPDFファイルについて:**
> スクリプトによって出力された `.pdf` ファイルは、プログラムの「出力」とみなされるため、**AGPL-3.0 ライセンスの対象外です。**
> したがって、ユーザーが生成したPDFファイルの著作権はユーザー自身に帰属し、配布・販売・私的利用を含め、**任意のライセンスで自由に扱うことができます。**
> （※ リリースしているオリジナルのテンプレートPDFが CC0 であることとは無関係に、ユーザーの生成物の権利はユーザー自身が保持します）
...
[Releasesページ](https://github.com/Kurato-Tsukishiro/Synapsen/releases) で配布されている **`.pdf` テンプレートファイル**（`DotLegalPad_Template-A4_Form.pdf` など）は、**CC0 (パブリックドメイン)** です。

これらのテンプレートは著作権による制限がないため、これらを使用して作成された**ユーザーのノートの著作権は完全にユーザー自身に帰属します。**
ユーザーはテンプレートのライセンスに縛られることなく、作成したノートを独占的に利用したり、あるいは配布・販売・共有したりと、自由に扱うことができます。
