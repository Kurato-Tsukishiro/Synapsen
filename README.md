...
* **データ保全・復旧機能:**
    ...
    * **メタデータ同期 (DB→PDF):**
        ...
        * **同期ロジック:** DB上の各ノートの「最終更新日時 (`updated_at`)」と、PDF内に記録された「前回同期日時」を比較し、変更があるファイルのみを高速に処理します。
...
* **【重要】ユーティリティボタンの表示について:**
    * 「**設定 (Config)**」、「**DB復旧ツール**」および「**メタデータ同期 (DB→PDF)**」ボタンは、ウィンドウ上部のツールバー右端に配置されています。
    * **ウィンドウサイズによってはボタンが隠れている場合があります。ボタンが見当たらない場合は、ウィンドウの横幅を広げて表示させてください。**
