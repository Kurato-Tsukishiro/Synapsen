...
* **補遺:**
    ...
    * **PDFの参照先:**
        * Nexusは DB に記録されたファイル名に基づき、以下の優先順位でPDF（統合PDFまたは元ファイル）を自動的に検索します。
            1. **`pdf_root_folder`** (config.ini):
               - メインフォルダ。ここを起点に**サブフォルダ（年度フォルダなど）も含めて再帰的に**検索します。
            2. **`pdf_archive_folder`** (config.ini):
               - アーカイブフォルダ。メインで見つからない場合に検索されます（サブフォルダ含む）。
            3. **DBの場所**:
               - 統合PDFがDBと同じフォルダにある場合などのフォールバック。
    * 検索文字列の例 :
        * `fulltext:Python`
